...
class ReplayMemory():
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    # More robust numerical safety checks
    max_safe_value = np.finfo(np.float32).max / (10 * batch_size)  # Extra conservative margin
    safe_p_total = min(p_total, max_safe_value)
    
    # Ensure segment_length is positive and reasonable
    segment_length = max(1e-6, safe_p_total / batch_size)  # Minimum segment length
    segment_starts = np.linspace(0, safe_p_total - segment_length, batch_size, dtype=np.float32)
    ...
    while not valid and attempts < max_attempts:
      attempts += 1
      # Generate samples with additional safety checks
      samples = np.clip(
          np.random.uniform(0.0, segment_length, [batch_size]).astype(np.float32) + segment_starts,
          0.0, safe_p_total - 1e-6  # Clip to safe range
      )
  ...
  def update_priorities(self, idxs, priorities):
    # Clip and normalize priorities to prevent numerical issues
    priorities = np.clip(priorities, 1e-6, 1e6)  # Prevent extreme values
    priorities = np.power(priorities, min(self.priority_exponent, 1.0))  # Cap exponent
    self.transitions.update(idxs, priorities.astype(np.float32))  # Ensure float32
