...
class HistoryPreprocessor:
    ...
    def process_state_for_network(self, state):
        """Process state for network input - returns state unchanged."""
        return state
...
class AtariPreprocessor:
    ...
    def process_state_for_memory(self, state):
        ...
        if state.ndim != 3 or state.shape[2] != 3:
            raise ValueError(f"Expected RGB image, got shape {state.shape}")
        if state.dtype != np.uint8:
            state = state.astype(np.uint8)
            
        I = Image.fromarray(state, 'RGB')
        I = I.convert('L')  # to gray
        I = I.resize((self.new_size, self.new_size), Image.Resampling.LANCZOS)
        ...
        # Convert from HWC to CHW format expected by PyTorch
        return np.transpose(I, (2, 0, 1)) if I.ndim == 3 else I[np.newaxis, ...]
    ...
    def process_reward(self, reward):
        """Return raw reward value for analysis"""
        return float(reward/50)#dealing with reward scaling

    def reset(self):
        """Reset the preprocessor state.
        AtariPreprocessor doesn't maintain state between episodes,
        so this is a no-op but provided for interface consistency.
        """
        pass
...
class PreprocessorSequence:
    ...
    def process_reward(self, reward):
        """Process the reward using the first preprocessor's method."""
        if len(self.preprocessors) > 0:
            return self.preprocessors[0].process_reward(reward)
        return reward
